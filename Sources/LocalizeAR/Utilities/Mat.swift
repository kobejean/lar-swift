...
extension Mat {
	...
	public func toSIMD3x3() -> simd_float3x3 {
		return simd_float3x3(
			.init(self.at(row: 0, col: 0).v, self.at(row: 1, col: 0).v, self.at(row: 2, col: 0).v),
			.init(self.at(row: 0, col: 1).v, self.at(row: 1, col: 1).v, self.at(row: 2, col: 1).v),
			.init(self.at(row: 0, col: 2).v, self.at(row: 1, col: 2).v, self.at(row: 2, col: 2).v)
		)
	}
